...
export default function Index() {
  ...
  const loadUserProfilePhoto = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('user_profiles').select('profile_photo_url').eq('user_id', user.id).maybeSingle();
      if (error) {
  ...
  return <div className="min-h-screen w-full bg-gradient-to-b from-[#3b9fe6] to-[#2a8dd9] flex flex-col overflow-hidden relative">
      {/* Fixed Header */}
      <header className="fixed top-0 w-full px-4 py-3 flex justify-between items-center bg-blue-500/80 backdrop-blur z-50">
        <button onClick={handleAvatarClick} className="transition-transform active:scale-95" aria-label="Go to user profile">
          <img src={profilePhotoUrl || AVATAR_SRC} className="w-10 h-10 rounded-full object-cover border-2 border-white/30 shadow hover:border-white/50 transition-colors" alt="User avatar" />
        </button>
      ...
      <div className={cn("absolute left-1/2 transform -translate-x-1/2 z-[100] transition-all duration-300 ease-in-out", isKeyboardVisible ? "opacity-0 pointer-events-none scale-95 translate-y-4" : "opacity-100 scale-100 translate-y-0")} style={{
      bottom: "50px"
    }}>
        <button className="w-12 h-12 rounded-full border-3 border-white bg-[#ffc000] shadow-xl flex items-center justify-center hover:scale-105 active:scale-95 transition-transform duration-150" style={{
        boxShadow: "0 8px 32px #ffc00040"
      }} aria-label="Voice input" onClick={handleMic} type="button">
      ...
      <div className="sticky bottom-0 z-50 w-full backdrop-blur-md shadow-lg">
        {/* Input Bar */}
        <InputBar value={input} onChange={setInput} onSend={handleSend} onHeightChange={setFooterHeight} onMicClick={handleMic} />

        {/* Bottom Navigation - Hidden when keyboard is visible */}
        <AppBottomTabs activeTab={activeTab} onTabChange={setActiveTab} isKeyboardVisible={isKeyboardVisible} />
      </div>
    </div>;
}