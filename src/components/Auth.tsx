...
export function AuthScreen() {
  ...
  const emailError = touched && !email ? "Please enter your email" : touched && !validateEmail(email) ? "Please enter a valid email address" : "";
      ...
      <div className="relative w-full max-w-sm mx-auto flex flex-col items-center z-10 px-4 py-8 rounded-xl shadow-lg bg-white/80">
        ...
        <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit} aria-labelledby="login-title">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input id="email" type="email" name="email" aria-required="true" aria-invalid={!!emailError} aria-describedby={emailError ? "email-error" : undefined} required autoFocus value={email} disabled={magicSent || loading} onBlur={() => setTouched(true)} onChange={e => {
            setEmail(e.target.value);
            setTouched(true);
          }} placeholder="you@example.com" className="text-base px-4 py-3 bg-white border border-[#eaeaea] focus-visible:ring-2 focus-visible:ring-[#3b9fe6]" />
            {emailError && <span className="text-xs text-red-500 mt-1" id="email-error">
                {emailError}
              </span>}
          </div>
          <Button type="submit" className={`mt-2 w-full font-bold text-base shadow active:scale-[0.98] transition-transform bg-[#3b9fe6] hover:bg-[#2176bd] focus-visible:ring-[#3b9fe6]`} disabled={magicSent || loading || !!emailError} aria-busy={loading}>
            {loading ? <span className="flex items-center justify-center gap-2">
                <Loader2 className="animate-spin" /> Sending...
              </span> : magicSent ? "Check your email!" : "Send Magic Link"}
          </Button>
        </form>
        {magicSent && <div className="mt-3 text-center text-sm text-[#3b9fe6] font-medium">
            Magic link sent<br />
