...
export function AuthScreen() {
  ...
  const confirmPasswordError = isSignUp && touched.confirm && confirmPassword !== password ? "Passwords do not match" : "";
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !validateEmail(email) || !password || !validatePassword(password) || isSignUp && confirmPassword !== password) {
      return;
    }
    setLoading(true);
    if (isSignUp) {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#eaeaea] via-white to-[#eaeaea] px-4 relative">
      ...
      <div className="relative w-full max-w-sm mx-auto flex flex-col items-center z-10 px-4 py-8 rounded-xl shadow-lg bg-white/80">
        ...
        <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit} aria-labelledby="login-title">
          ...
          <div className="flex flex-col gap-1 relative">
            ...
            <div className="relative">
              <Input id="password" type={showPassword ? "text" : "password"} name="password" autoComplete={isSignUp ? "new-password" : "current-password"} required disabled={loading} value={password} aria-required="true" aria-invalid={!!passwordError} aria-describedby={passwordError ? "password-error" : undefined} onBlur={() => setTouched(t => ({
              ...t,
              password: true
            }))} onChange={e => {
          ...
          {isSignUp && <div className="flex flex-col gap-1">
              ...
              <Input id="confirm-password" type={showPassword ? "text" : "password"} name="confirm-password" autoComplete="new-password" required disabled={loading} value={confirmPassword} aria-required="true" aria-invalid={!!confirmPasswordError} aria-describedby={confirmPasswordError ? "confirm-password-error" : undefined} onBlur={() => setTouched(t => ({
            ...t,
            confirm: true
          }))} onChange={e => {
          ...
          }} placeholder="Re-enter your password" className="text-base px-4 py-3 bg-white border border-[#eaeaea] focus-visible:ring-2 focus-visible:ring-[#3b9fe6]" />
              {confirmPasswordError && <span className="text-xs text-red-500 mt-1" id="confirm-password-error">{confirmPasswordError}</span>}
            </div>}
          <Button type="submit" className="mt-2 w-full font-bold text-base shadow active:scale-[0.98] transition-transform bg-[#3b9fe6] hover:bg-[#2176bd] focus-visible:ring-[#3b9fe6]" disabled={loading || !!emailError || !!passwordError || isSignUp && !!confirmPasswordError} aria-busy={loading}>
        ...
        <div className="mt-3 text-center text-sm text-[#3b9fe6] font-medium">
          ...
          <button className="ml-2 text-xs text-[#3b9fe6] underline hover:text-[#2176bd] transition-colors" type="button" onClick={() => {
          setIsSignUp(s => !s);
          setTouched({
            email: false,
            password: false,
